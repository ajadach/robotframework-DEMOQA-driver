from DemoQADriverPackageSelenium.demoqa_driver_package import DemoQADriverPackageSelenium
from robot.libraries.BuiltIn import BuiltIn
from robot.libraries.BuiltIn import RobotNotRunningError

from DemoQADriverPackageSelenium.Forms import forms
from DemoQADriverPackageSelenium.Elements import elements
from DemoQADriverPackageSelenium.Elements.TextBox import TextBox
import_text_box = 'Elements.TextBox'
from DemoQADriverPackageSelenium.Elements.CheckBox import CheckBox
import_check_box = 'Elements.CheckBox'
from DemoQADriverPackageSelenium.Elements.WebTables import WebTables
import_web_tables = 'Elements.WebTables'

TEXTBOX = TextBox()

__all__ = (
    'forms',
    'elements',
    import_text_box,
    import_check_box,
    import_web_tables,

)

try:
    for name in __all__:
        RFW = BuiltIn()
        RFW.import_library(f'DemoQADriverPackageSelenium.{name}')
except RobotNotRunningError:
    pass
